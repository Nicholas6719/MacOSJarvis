...
# stuck permission dialog can't hang Jarvis indefinitely.
...
def _read_events_script(start_dt: datetime.datetime, end_dt: datetime.datetime) -> str:
    ...
    across every calendar. Uses bulk property fetches (summary of theEvents,
    start date of theEvents) rather than per-event property loops — 10-100x
    faster on calendars with many events, which is essential for keeping
    Jarvis's background worker responsive."""
    return (
