...
# ── Pending calendar action ──────────────────────────────────────────────────
# Module-level state for multi-turn calendar flows. When Jarvis asks a
# clarifying question (e.g. "where are you working?"), this is set to a dict
# describing what we're waiting for. The next user utterance is interpreted
# as the answer to that question rather than a new command.
#
#   {
#     "type": "create_event",
#     "data": { ...all collected fields so far... },
#     "waiting_for": "location",
#     "memory_key": "work_location",  # optional — where to persist the answer
#   }
pending_calendar_action: Optional[dict] = None
...
class VoiceAssistant:
    ...
    # When we detect one of these in the utterance and no location is
    # provided, we look up the memory key first; if missing, we ask and
    # save the answer so next time Jarvis knows silently.
    _LOCATION_MEMORY_MAP = (
        (("working", "shift", "at work", "my shift", "work today"),
    ...
    def _detect_calendar_intent(self, text: str) -> Optional[str]:
        ...
        # ── Create reminder ──────────────────────────────────────────────
        # Must NOT match timer-style "remind me in 5 minutes" — that's
    ...
    def _llm_silent(self, system: str, user_prompt: str,
        ...
        """One-shot, non-streaming LLM call that does NOT touch history.
        Returns cleaned text. Used for calendar summaries, confirmations,
        and JSON extraction so system-crafted prompts never pollute the
    ...
    def _llm_speak(self, system: str, user_prompt: str,
                   max_tokens: int = 200) -> None:
        """Silent LLM call, then speak the result via TTS."""
        text = self._llm_silent(system, user_prompt, max_tokens=max_tokens)
        if text:
    ...
    def _handle_read_today(self) -> None:
        """Summarize today's calendar events via the LLM and speak them."""
        try:
            events = calendar_reminders.get_today_events()
        except Exception as e:
            print(f"[Calendar] get_today_events error: {e}")
            self.speak_direct(
            ...
            return
        if not events:
            self.speak_direct("Your calendar is clear for today, Sir.")
            return
        lines = []
        for e in events:
            parts = [f"- {e.get('title', '')}"]
        ...
        self._llm_speak(self._JARVIS_CAL_SYSTEM, prompt, max_tokens=220)

    def _handle_read_upcoming(self) -> None:
        """Summarize events from today through end of this coming Saturday."""
        try:
            events = calendar_reminders.get_upcoming_events()
        except Exception as e:
            print(f"[Calendar] get_upcoming_events error: {e}")
            self.speak_direct(
                "I wasn't able to access your calendar, Sir — "
                "you may need to grant permission in System Settings."
            )
        ...
        if not events:
            self.speak_direct(
                "Nothing on the books for the rest of the week, Sir."
            )
            return
        lines = []
        for e in events:
        ...
        self._llm_speak(self._JARVIS_CAL_SYSTEM, prompt, max_tokens=260)

    def _handle_read_reminders(self) -> None:
        """Summarize the user's open reminders via the LLM."""
        try:
            reminders = calendar_reminders.get_all_reminders()
        except Exception as e:
            print(f"[Calendar] get_all_reminders error: {e}")
            self.speak_direct(
                "I wasn't able to access your reminders, Sir — "
                "you may need to grant permission in System Settings."
            )
            return
        if not reminders:
            self.speak_direct("You have no open reminders, Sir.")
            return
        lines = []
        for r in reminders:
            parts = [f"- {r.get('title', '')}"]
            ...
            lines.append(" ".join(parts))
        prompt = (
            "These are the user's open reminders. Summarize them naturally and "
            "conversationally in Jarvis's voice — not a list, not a script, "
            "just how a sharp assistant would say them out loud. Keep it brief, "
    ...
    def _extract_event_json(self, utterance: str) -> Optional[dict]:
        ...
        utterance. Returns a dict with the fields described in the prompt,
        or None if extraction failed / JSON was malformed."""
        ...
        if not raw:
            return None
        # Strip code fences and leading text if the LLM got chatty.
        raw = re.sub(r"^```(?:json)?\s*", "", raw.strip())
        raw = re.sub(r"\s*```\s*$", "", raw).strip()
        # Try to grab the first {...} block if there's surrounding text.
    ...
    def _resolve_relative_date(self, date_str: str) -> datetime.date:
        """Resolve a date field that may be YYYY-MM-DD, today, tomorrow,
        or '(next) <weekday>'. Falls back to today if nothing matches."""
        ...
        if m:
            ...
            if delta == 0:
                delta = 7 if force_next else 0
            elif force_next and delta < 7:
                # "next Monday" when today is Sunday → usually means the one
                # after this coming one. Leave simple delta for now.
                pass
            return today + datetime.timedelta(days=delta)
        return today

    def _parse_time(self, t: Optional[str]) -> Optional[tuple]:
        """Parse an HH:MM 24-hour time string into a (hour, minute) tuple."""
        if not t:
            return None
        s = str(t).strip()
    ...
    def _infer_location_memory_key(self, utterance: str) -> Optional[tuple]:
        """If the utterance describes an event type where location is
        typically needed (working, gym, dentist, doctor…), return a
        (memory_key, clarifying_question) tuple. Otherwise return None,
        meaning 'don't ask about location — create the event without one'."""
    ...
    def _handle_create_event(self, user_input: str) -> None:
        """Extract, classify, resolve memory-backed location, and either
        ask a clarifying question or create the event."""
        ...
        try:
            # is_reminder override: if the LLM flagged it as a reminder,
            # route through the reminder flow instead.
            if bool(data_json.get("is_reminder")):
                self._handle_create_reminder(user_input, pre_extracted=data_json)
                return
            ...
            # Memory-backed location logic: only ask if this event type
            # is one where location matters and we don't already know it.
            if not location:
                ...
                if inferred is not None:
                    ...
                    if stored:
                        ...
                        return
                # else: no inferred key — location is optional, skip asking.

            self._finalize_create_event(data)
        except Exception as e:
            print(f"[Calendar] _handle_create_event error: {e}")
            self.speak_direct(
                "I wasn't able to process that calendar request, Sir."
            )

    def _finalize_create_event(self, data: dict) -> None:
        """Actually call AppleScript to create the event, then speak a
        natural LLM-generated confirmation."""
        try:
        ...
        self._llm_speak(self._JARVIS_CAL_SYSTEM, confirm_prompt, max_tokens=160)

    def _handle_create_reminder(
        self, user_input: str, pre_extracted: Optional[dict] = None
    ) -> None:
        """Extract fields, create the reminder, speak a confirmation."""
        ...
        self._llm_speak(self._JARVIS_CAL_SYSTEM, confirm_prompt, max_tokens=140)
    ...
    def _handle_calendar_intent(self, intent: str, user_input: str) -> None:
        ...
        try:
            if intent == "read_today":
                ...
                self._handle_read_upcoming()
            elif intent == "read_reminders":
                self._handle_read_reminders()
            elif intent == "create_event":
                self._handle_create_event(user_input)
    ...
    def _resume_pending_calendar_action(self, answer: str) -> None:
        ...
        to the pending action and finish it."""
        global pending_calendar_action
        pending = pending_calendar_action
        pending_calendar_action = None  # Clear before any further await
        if not pending:
            return
        ...
        try:
            ...
            if waiting_for == "location":
                ...
                if not loc:
                    self.speak_direct("I didn't catch a location, Sir. Try again.")
                    return
                # Persist silently so next time Jarvis knows.
                mem_key = pending.get("memory_key")
                if mem_key:
                    try:
                        memory.save_fact(mem_key, loc)
                        self._rebuild_system_prompt()
                        print(f"[Memory] Saved {mem_key} = {loc}")
                    except Exception as e:
                        print(f"[Memory] save location error: {e}")
                data["location"] = loc
            ...
            self.speak_direct("I'm not sure what to do with that, Sir.")
        except Exception as e:
            print(f"[Calendar] resume error: {e}")
            self.speak_direct(
                "Something went wrong finishing that calendar action, Sir."
            )
    ...
    def run(self) -> None:
        ...
        while True:
            try:
                ...
                # Pending calendar clarification: treat this utterance as
                # the answer to the last question rather than a new command.
                if pending_calendar_action is not None:
